...
class navigator:
    ...
    def clearSearch(self):
        ...
        confirm = xbmcgui.Dialog().yesno("Ikigai", "Clear Search History?")
        if confirm == True:
            cache.cache_clear_search()
            dialog = xbmcgui.Dialog()
            dialog.notification('Ikigai', 'Search History Cleared', xbmcgui.NOTIFICATION_INFO, 5000)

    def clearCache(self):
        confirm = xbmcgui.Dialog().yesno("Ikigai", "Clear Cache?")
        if confirm == True:
            from lib import cache
            cache.cache_clear_all()
            dialog = xbmcgui.Dialog()
            dialog.notification('Ikigai', 'Cleared Cache', xbmcgui.NOTIFICATION_INFO, 5000)
...
class player:
    ...
    def playItem(self, slug, url):
        ...
        if len(link_list) == 0:
            ...
            dialog = xbmcgui.Dialog()
            dialog.notification('Ikigai', 'No Links Available', xbmcgui.NOTIFICATION_INFO, 5000)

        else:

            resolve_dialog.create('Ikigai', '')
            ...
            if path != '':
                ...
                dialog = xbmcgui.Dialog()
                dialog.notification('Ikigai', 'Unable to Resolve Links', xbmcgui.NOTIFICATION_INFO, 5000)